...
def runClock(BRIGHTNESS):
    ...
    while runClock:
        scrollphathd.clear()       
        if currThread.stopped(): # if we are running in thread then check if it was not stopped
            runClock = False
            continue        
        ...
        if DISPLAY_BAR:
            ...
            for y in range(15):
                ...
                scrollphathd.set_pixel(y + 1, 6, current_pixel * float(BRIGHTNESS))
            ...
            scrollphathd.set_pixel(int(seconds_progress), 6, float(BRIGHTNESS))
        ...
        scrollphathd.write_string(
            ...
            brightness=float(BRIGHTNESS)  # Use our global brightness value
        )
